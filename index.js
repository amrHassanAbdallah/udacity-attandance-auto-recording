...
async function getParticipantsFromZoom (page) {
  await page.goto(config.zoom.loginURL,
  ...
  //get the report data
  await page.goto(
    config.zoom.reportsURL + getReportsParamsArgs(config.attendanceDate),
    { waitUntil: 'networkidle0' })
  try {
    await page.waitForSelector('a[data-attendees]')
  } catch (error) {
    console.log('No records found in this \'' + config.attendanceDate +
...
(async () => {
  ...
  //go to udacity
  //fill the attendance
  await page.goto(config.udacity.loginURL,
    { waitUntil: 'networkidle0', timeout: 60000 })
